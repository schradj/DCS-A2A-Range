...
--- **Functional** - A2A Range Practice.
--
-- ===
--
-- The A2A_RANGE class enables easy set up Player vs AI air combat ranges within DCS World.
--
-- **Main Features:**
--
-- ===
...
-- @field #string rangename Name of the A2ARANGE.
...
-- @type A2ARANGE.Defaults
A2A_RANGE.Defaults = {
}

--- Sound file data.
-- @type A2ARANGE.Soundfile
-- @field #string filename Name of the file
-- @field #number duration Duration in seconds.

--- Sound files.
-- @type A2ARANGE.Sound
-- @field #A2ARANGE.Soundfile RC0
-- @field #A2ARANGE.Soundfile RC1
-- @field #A2ARANGE.Soundfile RC2
-- @field #A2ARANGE.Soundfile RC3
-- @field #A2ARANGE.Soundfile RC4
-- @field #A2ARANGE.Soundfile RC5
-- @field #A2ARANGE.Soundfile RC6
-- @field #A2ARANGE.Soundfile RC7
-- @field #A2ARANGE.Soundfile RC8
-- @field #A2ARANGE.Soundfile RC9
-- @field #A2ARANGE.Soundfile RCAccuracy
-- @field #A2ARANGE.Soundfile RCDegrees
-- @field #A2ARANGE.Soundfile RCExcellentHit
-- @field #A2ARANGE.Soundfile RCExcellentPass
-- @field #A2ARANGE.Soundfile RCFeet
-- @field #A2ARANGE.Soundfile RCFor
-- @field #A2ARANGE.Soundfile RCGoodHit
-- @field #A2ARANGE.Soundfile RCGoodPass
-- @field #A2ARANGE.Soundfile RCHitsOnTarget
-- @field #A2ARANGE.Soundfile RCImpact
-- @field #A2ARANGE.Soundfile RCIneffectiveHit
-- @field #A2ARANGE.Soundfile RCIneffectivePass
-- @field #A2ARANGE.Soundfile RCInvalidHit
-- @field #A2ARANGE.Soundfile RCLeftStrafePitTooQuickly
-- @field #A2ARANGE.Soundfile RCPercent
-- @field #A2ARANGE.Soundfile RCPoorHit
-- @field #A2ARANGE.Soundfile RCPoorPass
-- @field #A2ARANGE.Soundfile RCRollingInOnStrafeTarget
-- @field #A2ARANGE.Soundfile RCTotalRoundsFired
-- @field #A2ARANGE.Soundfile RCWeaponImpactedTooFar
-- @field #A2ARANGE.Soundfile IR0
-- @field #A2ARANGE.Soundfile IR1
-- @field #A2ARANGE.Soundfile IR2
-- @field #A2ARANGE.Soundfile IR3
-- @field #A2ARANGE.Soundfile IR4
-- @field #A2ARANGE.Soundfile IR5
-- @field #A2ARANGE.Soundfile IR6
-- @field #A2ARANGE.Soundfile IR7
-- @field #A2ARANGE.Soundfile IR8
-- @field #A2ARANGE.Soundfile IR9
-- @field #A2ARANGE.Soundfile IRDecimal
-- @field #A2ARANGE.Soundfile IRMegaHertz
-- @field #A2ARANGE.Soundfile IREnterRange
-- @field #A2ARANGE.Soundfile IRExitRange
...
A2ARANGE.MenuF10Root = nil

--- Range script version.
-- @field #string version
A2A_RANGE.version = "0.1.0"
...
-- @param #RANGE self
-- @param #string RangeName Name of the A2ARANGE. Has to be unique. Will we used to create F10 menu items etc.
-- @return #RANGE RANGE object.
function A2A_RANGE:New(RangeName, WarehouseName)
  ...
  self.rangename = RangeName or "A2A Range"

  -- Log id.
  self.lid = string.format( "A2A_RANGE %s | ", self.rangename )

  -- Debug info.
  local text = string.format( "Script version %s - creating new A2A_RANGE object %s.", A2ARANGE.version, self.rangename )
  ...
  self:AddTransition("*",               "EnterRange",        "*")           -- Player enters the A2A_RANGE.
  self:AddTransition("*",               "ExitRange",         "*")           -- Player leaves the A2A_RANGE.
  ...
  --- Triggers the FSM event "Start". Starts the A2ARANGE. Initializes parameters and starts event handlers.
  -- @function [parent=#RANGE] Start
  -- @param #RANGE self

  --- Triggers the FSM event "Start" after a delay. Starts the A2ARANGE. Initializes parameters and starts event handlers.
  ...
  -- @param #RANGE self
  -- @param #A2ARANGE.PlayerData player Data of player settings etc.
...
--- Initializes number of targets and location of the A2A_RANGE. Starts the event handlers.
-- @param #RANGE self
-- @param #string From From state.
-- @param #string Event Event.
-- @param #string To To state.
function RANGE:onafterStart()
  ...
  -- Location/coordinate of A2ARANGE.
  local _location = nil

  -- Location of the A2ARANGE. We simply take the first unit/target we find if it was not explicitly specified by the user.
  ...
  -- Define a MOOSE zone of the A2ARANGE.
  ...
  for _, _target in pairs( self.bombingTargets ) do
    local target=_target --#A2ARANGE.BombTarget

    -- Check if unit and can move.
    if target.move and target.type==A2ARANGE.TargetType.UNIT and target.speed > 1 then
  ...
  if self.rangecontrolfreq and not self.useSRS then
    ...
    -- Init numbers.
    self.rangecontrol:SetDigit( 0, A2ARANGE.Sound.RC0.filename, A2ARANGE.Sound.RC0.duration, self.soundpath )
    self.rangecontrol:SetDigit( 1, A2ARANGE.Sound.RC1.filename, A2ARANGE.Sound.RC1.duration, self.soundpath )
    self.rangecontrol:SetDigit( 2, A2ARANGE.Sound.RC2.filename, A2ARANGE.Sound.RC2.duration, self.soundpath )
    self.rangecontrol:SetDigit( 3, A2ARANGE.Sound.RC3.filename, A2ARANGE.Sound.RC3.duration, self.soundpath )
    self.rangecontrol:SetDigit( 4, A2ARANGE.Sound.RC4.filename, A2ARANGE.Sound.RC4.duration, self.soundpath )
    self.rangecontrol:SetDigit( 5, A2ARANGE.Sound.RC5.filename, A2ARANGE.Sound.RC5.duration, self.soundpath )
    self.rangecontrol:SetDigit( 6, A2ARANGE.Sound.RC6.filename, A2ARANGE.Sound.RC6.duration, self.soundpath )
    self.rangecontrol:SetDigit( 7, A2ARANGE.Sound.RC7.filename, A2ARANGE.Sound.RC7.duration, self.soundpath )
    self.rangecontrol:SetDigit( 8, A2ARANGE.Sound.RC8.filename, A2ARANGE.Sound.RC8.duration, self.soundpath )
    self.rangecontrol:SetDigit( 9, A2ARANGE.Sound.RC9.filename, A2ARANGE.Sound.RC9.duration, self.soundpath )
    ...
    if self.instructorfreq and not self.useSRS then
      ...
      -- Init numbers.
      self.instructor:SetDigit( 0, A2ARANGE.Sound.IR0.filename, A2ARANGE.Sound.IR0.duration, self.soundpath )
      self.instructor:SetDigit( 1, A2ARANGE.Sound.IR1.filename, A2ARANGE.Sound.IR1.duration, self.soundpath )
      self.instructor:SetDigit( 2, A2ARANGE.Sound.IR2.filename, A2ARANGE.Sound.IR2.duration, self.soundpath )
      self.instructor:SetDigit( 3, A2ARANGE.Sound.IR3.filename, A2ARANGE.Sound.IR3.duration, self.soundpath )
      self.instructor:SetDigit( 4, A2ARANGE.Sound.IR4.filename, A2ARANGE.Sound.IR4.duration, self.soundpath )
      self.instructor:SetDigit( 5, A2ARANGE.Sound.IR5.filename, A2ARANGE.Sound.IR5.duration, self.soundpath )
      self.instructor:SetDigit( 6, A2ARANGE.Sound.IR6.filename, A2ARANGE.Sound.IR6.duration, self.soundpath )
      self.instructor:SetDigit( 7, A2ARANGE.Sound.IR7.filename, A2ARANGE.Sound.IR7.duration, self.soundpath )
      self.instructor:SetDigit( 8, A2ARANGE.Sound.IR8.filename, A2ARANGE.Sound.IR8.duration, self.soundpath )
      self.instructor:SetDigit( 9, A2ARANGE.Sound.IR9.filename, A2ARANGE.Sound.IR9.duration, self.soundpath )
...
function RANGE:SetMaxStrafeAlt( maxalt )
  self.strafemaxalt = maxalt or A2ARANGE.Defaults.strafemaxalt
  return self
end
...
function RANGE:SetBombtrackTimestep( dt )
  self.dtBombtrack = dt or A2ARANGE.Defaults.dtBombtrack
  return self
end
...
function RANGE:SetMessageTimeDuration( time )
  self.Tmsg = time or A2ARANGE.Defaults.Tmsg
  return self
end
...
function RANGE:SetDisplayedMaxPlayerResults( nmax )
  self.ndisplayresult = nmax or A2ARANGE.Defaults.ndisplayresult
  return self
end
...
function RANGE:SetRangeRadius( radius )
  self.rangeradius = radius * 1000 or A2ARANGE.Defaults.rangeradius
  return self
end
...
--- Set range location. If this is not done, one (random) unit position of the range is used to determine the location of the A2ARANGE.
-- The range location determines the position at which the weather data is evaluated.
-- @param #RANGE self
-- @param Core.Point#COORDINATE coordinate Coordinate of the A2ARANGE.
-- @return #RANGE self
...
function RANGE:SetSmokeTimeDelay( delay )
  self.TdelaySmoke = delay or A2ARANGE.Defaults.TdelaySmoke
  return self
end
...
function RANGE:AddStrafePit( targetnames, boxlength, boxwidth, heading, inverseheading, goodpass, foulline )
  ...
  local l = boxlength or A2ARANGE.Defaults.boxlength
  local w = (boxwidth or A2ARANGE.Defaults.boxwidth) / 2
  ...
  goodpass = goodpass or A2ARANGE.Defaults.goodpass

  -- Foule line distance.
  foulline = foulline or A2ARANGE.Defaults.foulline

  -- Coordinate of the A2ARANGE.
  ...
  local st = {} -- #A2ARANGE.StrafeTarget
  st.name = _name
  st.polygon = _polygon
...
--- Add bombing target(s) to A2ARANGE.
-- @param #RANGE self
...
-- @param #boolean randommove If true, unit will move randomly within the A2ARANGE. Default is false.
-- @return #RANGE self
function RANGE:AddBombingTargets( targetnames, goodhitrange, randommove )
  ...
  goodhitrange = goodhitrange or A2ARANGE.Defaults.goodhitrange
...
function RANGE:AddBombingTargetUnit( unit, goodhitrange, randommove )
  ...
  local target = {} -- #A2ARANGE.BombTarget
  target.name = name
  target.target = unit
  ...
  if _isstatic then
    target.type = A2ARANGE.TargetType.STATIC
  else
    target.type = A2ARANGE.TargetType.UNIT
  end
...
function RANGE:AddBombingTargetCoordinate( coord, name, goodhitrange )
  ...
  target.target = nil
  target.goodhitrange = goodhitrange or A2ARANGE.Defaults.goodhitrange
  target.move = false
  target.speed = 0
  target.coordinate = coord
  target.type = A2ARANGE.TargetType.COORD
...
function RANGE:AddBombingTargetScenery( scenery, goodhitrange)
  ...
  target.type = A2ARANGE.TargetType.SCENERY
...
function RANGE:OnEventBirth( EventData )
  ...
  if _unit and _playername then
    ...
    self.PlayerSettings[_playername] = {} -- #A2ARANGE.PlayerData
...
function RANGE:OnEventHit( EventData )
  ...
  local _currentTarget = self.strafeStatus[_unitID] --#A2ARANGE.StrafeStatus
...
-- @param #A2ARANGE.PlayerData playerData Player data table.
-- @param #number attackHdg Attack heading.
-- @param #number attackAlt Attack altitude.
-- @param #number attackVel Attack velocity.
function A2ARANGE._OnImpact(weapon, self, playerData, attackHdg, attackAlt, attackVel)

  -- Get closet target to last position.
  local _closetTarget = nil -- #A2ARANGE.BombTarget
  local _distance = nil
  ...
  for _, _bombtarget in pairs( self.bombingTargets ) do
    local bombtarget=_bombtarget  --#A2ARANGE.BombTarget
  ...
  if _distance and _distance <= self.scorebombdistance then
    ...
    local result = {} -- #A2ARANGE.BombResult
  ...
  elseif insidezone then
    ...
    if self.rangecontrol then
      ...
      if self.useSRS then
        ...
        self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCWeaponImpactedTooFar.filename, A2ARANGE.Sound.RCWeaponImpactedTooFar.duration, self.soundpath, nil, nil, _message, self.subduration )
      end
    end
...
function RANGE:OnEventShot( EventData )
  ...
  -- Distance Player-to-A2ARANGE. Set this to larger value than the threshold.
  local dPR = self.BombtrackThreshold * 2

  -- Distance player to A2ARANGE.
  if _unit and _playername then
  ...
  if _track and dPR <= self.BombtrackThreshold and _unit and _playername then
    ...
    -- Player data.
    local playerData = self.PlayerSettings[_playername] -- #A2ARANGE.PlayerData
    ...
    weapon:SetFuncImpact(A2ARANGE._OnImpact, self, playerData, attackHdg, attackAlt, attackVel)
...
-- @param #string Event Event.
-- @param #string To To state.
-- @param #A2ARANGE.PlayerData player Player data.
function RANGE:onafterEnterRange( From, Event, To, player )
  ...
  if self.instructor and self.rangecontrol then
    ...
    if self.useSRS then
      ...
      local text = string.format("You entered the bombing A2ARANGE. For hit assessment, contact the range controller at %.3f MHz", self.rangecontrolfreq)
      local ttstext = string.format("You entered the bombing A2ARANGE. For hit assessment, contact the range controller at %.3f mega hertz.", self.rangecontrolfreq)
      ...
      -- You entered the bombing A2ARANGE. For hit assessment, contact the range controller at xy MHz
      self.instructor:NewTransmission( A2ARANGE.Sound.IREnterA2ARANGE.filename, A2ARANGE.Sound.IREnterA2ARANGE.duration, self.soundpath )
      self.instructor:Number2Transmission( RF[1] )
      
      if tonumber( RF[2] ) > 0 then
        self.instructor:NewTransmission( A2ARANGE.Sound.IRDecimal.filename, A2ARANGE.Sound.IRDecimal.duration, self.soundpath )
        self.instructor:Number2Transmission( RF[2] )
      end
      
      self.instructor:NewTransmission( A2ARANGE.Sound.IRMegaHertz.filename, A2ARANGE.Sound.IRMegaHertz.duration, self.soundpath )
    end
  end
...
function RANGE:onafterExitRange( From, Event, To, player )
  ...
  if self.instructor then
    ...
    if self.useSRS then
      ...
      self.instructor:NewTransmission( A2ARANGE.Sound.IRExitA2ARANGE.filename, A2ARANGE.Sound.IRExitA2ARANGE.duration, self.soundpath )
    end
  end

end

--- Function called after bomb impact on A2ARANGE.
-- @param #RANGE self
-- @param #string From From state.
-- @param #string Event Event.
-- @param #string To To state.
-- @param #A2ARANGE.BombResult result Result of bomb impact.
-- @param #A2ARANGE.PlayerData player Player data table.
function RANGE:onafterImpact( From, Event, To, result, player )
  ...
  if self.rangecontrol then
    ...
    if self.useSRS then
      ...
      self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCImpact.filename, A2ARANGE.Sound.RCImpact.duration, self.soundpath, nil, nil, text, self.subduration )
      self.rangecontrol:Number2Transmission( string.format( "%03d", result.radial ), nil, 0.1 )
      self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCDegrees.filename, A2ARANGE.Sound.RCDegrees.duration, self.soundpath )
      self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCFor.filename, A2ARANGE.Sound.RCFor.duration, self.soundpath )
      self.rangecontrol:Number2Transmission( string.format( "%d", UTILS.MetersToFeet( result.distance ) ) )
      self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCFeet.filename, A2ARANGE.Sound.RCFeet.duration, self.soundpath )
      if result.quality == "POOR" then
        self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCPoorHit.filename, A2ARANGE.Sound.RCPoorHit.duration, self.soundpath, nil, 0.5 )
      elseif result.quality == "INEFFECTIVE" then
        self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCIneffectiveHit.filename, A2ARANGE.Sound.RCIneffectiveHit.duration, self.soundpath, nil, 0.5 )
      elseif result.quality == "GOOD" then
        self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCGoodHit.filename, A2ARANGE.Sound.RCGoodHit.duration, self.soundpath, nil, 0.5 )
      elseif result.quality == "EXCELLENT" then
        self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCExcellentHit.filename, A2ARANGE.Sound.RCExcellentHit.duration, self.soundpath, nil, 0.5 )
      end
    end
  end
...
-- @param #string To To state.
-- @param #A2ARANGE.PlayerData player Player data table.
-- @param #A2ARANGE.StrafeResult result Result of run.
...
function RANGE:onafterSave( From, Event, To )
  ...
  for playername, results in pairs( self.bombPlayerResults ) do
    ...
    for i, _result in pairs( results ) do
      local result = _result -- #A2ARANGE.BombResult
...
-- @param #A2ARANGE.StrafeResult result Results table.
...
function A2ARANGE._DelayedSmoke( _args )
  ...
  if _unit and _playername then
    ...
    if _results == nil then
      ...
      for _, _result in pairs( _results ) do
        local result=_result --#A2ARANGE.StrafeResult
...
--- Report information like bearing and range from player unit to A2ARANGE.
-- @param #RANGE self
-- @param #string _unitname Name of the player unit.
...
--- Report weather conditions at A2ARANGE. Temperature, QFE pressure and wind data.
-- @param #RANGE self
-- @param #string _unitname Name of the player unit.
...
function RANGE:_CheckPlayers()
  ...
  for playername, _playersettings in pairs( self.PlayerSettings ) do
    local playersettings = _playersettings -- #A2ARANGE.PlayerData
...
function RANGE:_CheckInZone( _unitName )
  ...
  if _unit and _playername then
    ...
    local _currentStrafeRun = self.strafeStatus[_unitID] --#A2ARANGE.StrafeStatus
    ...
    if _currentStrafeRun then -- player has already registered for a strafing run.
        ...
        if _currentStrafeRun.time <= 3 then
          ...
          if self.rangecontrol then
            if self.useSRS then
              ...
              self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCLeftStrafePitTooQuickly.filename, A2ARANGE.Sound.RCLeftStrafePitTooQuickly.duration, self.soundpath )
            end
          end
        else
          ...
          -- Result.
          local _result = self.strafeStatus[_unitID] --#A2ARANGE.StrafeStatus
          
          local _sound = nil -- #A2ARANGE.Soundfile
          ...
          -- Strafe result.
          local result = {} -- #A2ARANGE.StrafeResult
          ...
          if self.rangecontrol then
            if self.useSRS then
              ...
              self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCHitsOnTarget.filename, A2ARANGE.Sound.RCHitsOnTarget.duration, self.soundpath )
              self.rangecontrol:Number2Transmission( string.format( "%d", _result.hits ) )
              if shots and accur then
                self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCTotalRoundsFired.filename, A2ARANGE.Sound.RCTotalRoundsFired.duration, self.soundpath, nil, 0.2 )
                self.rangecontrol:Number2Transmission( string.format( "%d", shots ), nil, 0.2 )
                self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCAccuracy.filename, A2ARANGE.Sound.RCAccuracy.duration, self.soundpath, nil, 0.2 )
                self.rangecontrol:Number2Transmission( string.format( "%d", UTILS.Round( accur, 0 ) ) )
                self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCPercent.filename, A2ARANGE.Sound.RCPercent.duration, self.soundpath )
              end
      ...
      for _, _targetZone in pairs( self.strafeTargets ) do
        local target=_targetZone --#A2ARANGE.StrafeTarget
        ...
        if unitinzone then
          ...
          if self.rangecontrol then
            if self.useSRS then
              ...
              self.rangecontrol:NewTransmission( A2ARANGE.Sound.RCRollingInOnStrafeTarget.filename, A2ARANGE.Sound.RCRollingInOnStrafeTarget.duration, self.soundpath )
            end
          end
  ...
  if _unit and playername then
    ...
    if group and _gid then
      ...
      if not self.MenuAddedTo[_gid] then
        ...
        if A2ARANGE.MenuF10Root then

          -------------------
          -- MISSION LEVEL --
          -------------------

          -- _rangePath = missionCommands.addSubMenuForGroup(_gid, self.rangename, A2ARANGE.MenuF10Root)
          ...
          if A2ARANGE.MenuF10[_gid] == nil then
            -- A2ARANGE.MenuF10[_gid]=missionCommands.addSubMenuForGroup(_gid, "On the Range")
            A2ARANGE.MenuF10[_gid] = MENU_GROUP:New( group, "On the Range" )
          end
          -- _rangePath = missionCommands.addSubMenuForGroup(_gid, self.rangename, A2ARANGE.MenuF10[_gid])
          _rangePath = MENU_GROUP:New( group, self.rangename, A2ARANGE.MenuF10[_gid] )
        end
...
-- @param #RANGE self
-- @param #A2ARANGE.BombTarget target Bomb target data.
...
function RANGE:_GetBombTargetCoordinate( target )
  ...
  if target.type == A2ARANGE.TargetType.UNIT then
  ...
  elseif target.type == A2ARANGE.TargetType.STATIC then

    -- Static targets dont move.
    coord = target.coordinate

  elseif target.type == A2ARANGE.TargetType.COORD then

    -- Coordinates dont move.
    coord = target.coordinate
    
  elseif target.type == A2ARANGE.TargetType.SCENERY then
...
function RANGE:_TargetsheetOnOff( _unitname )
  ...
  if unit and playername then
    ...
    -- Player data.
    local playerData = self.PlayerSettings[playername] -- #A2ARANGE.PlayerData
    ...
    if playerData then
      ...
      if self.targetsheet then
        ...
        text = "negative, target sheet data recorder is broken on this A2ARANGE."
      end
...
function RANGE:_SmokeStrafeTargetBoxes( unitname )
  ...
  for _, _target in pairs( self.strafeTargets ) do
    ...
    for _, _point in pairs( _target.smokepoints ) do
      _point:SmokeOrange() -- Corners are smoked oA2ARANGE.
    end
  end
